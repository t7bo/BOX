...
class UpcomingMoviesPipeline:
    ...
    def process_item(self, item, spider):
        ...
        for field_name in field_names:
            ...
            if field_name == 'movie_url':
                ...
                if movie_original_title is not None:
                    if "Titre original\xa0:" in movie_original_title:
                ...
                if movie_imdb_nb_of_ratings is not None:
                    # Convert 1K to 1 000
                    if "k" in movie_imdb_nb_of_ratings:
                        movie_imdb_nb_of_ratings = int(''.join(filter(str.isdigit, movie_imdb_nb_of_ratings))) * 1000
                    # Convert 1M to 1 000 000
                    elif "M" in movie_imdb_nb_of_ratings:
                        movie_imdb_nb_of_ratings = int(''.join(filter(str.isdigit, movie_imdb_nb_of_ratings))) * 1000000
                ...
                if movie_us_boxoffice is not None:
                    ...
                    if " $US" in movie_us_boxoffice:
                        movie_us_boxoffice = movie_us_boxoffice.replace(" $US", "")
                    movie_us_boxoffice = int(movie_us_boxoffice)
                adapter['movie_us_boxoffice'] = movie_us_boxoffice
        ...
        self.cur.execute("""
                         INSERT INTO upcoming_movies (movie_id, movie_url, release_date, movie_title, movie_original_title, movie_length, movie_imdb_rating, movie_imdb_nb_of_ratings, movie_imdb_popularity, movie_synopsis, movie_director, movie_cast, movie_categories, movie_imdb_metascore, movie_countries, movie_production_companies, movie_budget, movie_us_boxoffice, movie_boxoffice, movie_poster) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                         """,
