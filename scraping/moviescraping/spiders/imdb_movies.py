...
class ImdbMoviesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        try:        
            ...
            movie['release_date'] = response.xpath('//section[@data-testid="Details"]/div[@data-testid="title-details-section"]/ul/li[1]/div/ul/li/a/text()').get()
            ...
            if poster_url:
                yield response.follow('https://www.imdb.com' + poster_url, callback=self.parse_poster_page, meta={'movie': movie})
            else:
                yield movie
    ...
    def parse_poster_page(self, response):
        ...
        movie['movie_poster'] = response.xpath('//img[@class="sc-7c0a9e7c-0 eWmrns"]/@src').get()
        yield movie
